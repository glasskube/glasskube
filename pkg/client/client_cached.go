...
func (c *cacheClientset) Packages() PackageInterface {
	p := c.PackageV1Alpha1Client.Packages()
...
func (c *readWriteCacheClient[T, L]) Create(ctx context.Context, target *T) error {
	return c.fallback.Create(ctx, target)
}
