...
func DescribePackage(
	ctx context.Context,
	pkgName string,
) (*v1alpha1.Package, *client.PackageStatus, *v1alpha1.PackageManifest, *[]v1alpha1.PackageEntrypoint, error) {
	...
	if err := pkgClient.Packages().Get(ctx, pkgName, &pkg); err == nil {
		...
		if installedManifest, err := manifest.GetInstalledManifestForPackage(ctx, pkg); err == nil {
			return &pkg, status, installedManifest, nil, nil
	...
	return &pkg, status, &packageManifest, &packageManifest.Entrypoints, nil
}
