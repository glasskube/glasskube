...
func (c *updater) VersionUpdate(ctx context.Context, pkgName string, pkgVersion string) (*UpdateTransaction, error) {
	c.status.Start()
	defer c.status.Stop()
	...
	if semver.IsUpgradable(pkg.Spec.PackageInfo.Version, pkgVersion) {
		...
		if result, err := c.dm.Validate(ctx, &manifest); err != nil {
			return nil, err
		} else if cf, err := c.dm.IsUpdateAllowed(ctx, &pkg, pkgVersion); err != nil {
			return nil, err
