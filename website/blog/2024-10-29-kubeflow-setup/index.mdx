---
slug: wip-kubeflow-setup
title: 'WIP: Kubeflow Setup Guide'
description: TODO
authors: [kosmoz]
...
- Up-to-date version of `gcloud` installed and initialized
...
Countless enterprises and organizations have adopted it as a platform to serve their internal and external IT infrastructure, benefiting from it's declarative management, self-healing and scaling capabilities.
...
Furthermore, AI workloads have very different performance characteristics compared to the web servers and database servers we are to.
They are, generally, very resource demanding, strongly coupled and often short lived.

While advanced technices, like cluster autoscaling, can help with some of these challenges, there is a clear need for Kubernetes primitives representing higher level concepts that AI researches and data scientists are more familiar with.
...
It also helps researches to reify their machine-learning workflows using "pipelines", leading to faster and more reproducible model development cycles.

### Kubeflow Platform Architecture

If all of that sounds to you like a very complex system with a lot of moving parts, for better of for worse, you are right.
Kubeflow is composed of several components and is built on a base of several third-party pojects:
...
Google cloud makes it very easy to create Kubernetes cluster with the Google Kubernetes Engine (GKE), but our installation has some special requirements that need to be taken care of. Here are our considerations:
...
1. The first node pool (let's call it `default-pool`, because that's the default) should be configured with
...
2. Add a second node pool using the button in the top bar (let's call this one `gpu-pool`) and configure it with
...
To use this manifest, you need to clone the repository locally, by running `git clone git@github.com:kubeflow/manifests.git`.
...
If you just want to see what Kubeflow looks like or experiment a little bit, you can already stop here.
Kubeflow is usable behind `kubectl port-forward`.
However, you probably intend other users to join the platform and expecting everyone to do this is not reasonable.
...
But since this service is now reachable for the public, you will probably want to set up a more robust authentication flow.
...
Like many other organizations, we use several of Microsofts producs, so it makes sense for us to use Azure Active Directory (AAD) here.
...
```yaml name="custom/microsoft-auth/kustomization.yaml"
kind: Component
patches:
...
- Add the `microsoft` connector, as seen below (replace the client ID, tenant ID and client secret with the ones you noted down before)
...
This is enough for Dex to work with the Microsoft IDP, but it now identifies as a different issuer internally, so we have to update the `oauth2-proxy` configuration to accept this issuer name. For this, update the file you created before, adding a patch for the `RequestAuthentication` resource:
...
Then restart the `dex` and `oauth2-proxy` deployments:

```sh
kubectl rollout restart -n auth deployment dex
kubectl rollout restart -n oauth2-proxy deployment oauth2-proxy
```
...
- Set `CD_REGISTRATION_FLOW` to true in `apps/centraldashboard/upstream/base/params.env`
