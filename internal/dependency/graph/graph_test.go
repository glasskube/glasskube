...
var _ = Describe("DependencyGraph", func() {
	...
	Describe("Validate", func() {
		...
		When("there is a package without dependencies", func() {
			It("should not return an error", func() {
				Expect(graph.Add(foo, "v1.0.0", nil, true)).NotTo(HaveOccurred())
				Expect(graph.Validate()).NotTo(HaveOccurred())
			})
		})
		...
		When("there is a package with dependencies", func() {
			When("the dependency exists", func() {
				When("there is a constraint", func() {
					It("should not return an error", func() {
						Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar}}, true)).NotTo(HaveOccurred())
						Expect(graph.Add(bar, "v1.0.0", nil, false)).NotTo(HaveOccurred())
						Expect(graph.Validate()).NotTo(HaveOccurred())
					})
				})
				...
				When("there is a constraint and it is not violated", func() {
					It("should not return an error", func() {
						Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: "1.x.x"}}, true)).NotTo(HaveOccurred())
						Expect(graph.Add(bar, "v1.0.0", nil, false)).NotTo(HaveOccurred())
						Expect(graph.Validate()).NotTo(HaveOccurred())
					})
				})
				...
				When("there is a constraint and it is violated", func() {
					It("should return an error", func() {
						Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: "1.1.x"}}, true)).NotTo(HaveOccurred())
						Expect(graph.Add(bar, "v1.0.0", nil, false)).NotTo(HaveOccurred())
	...
	Describe("Max", func() {
		It("should return error for empty slice", func() {
			Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.0.0, < 1.1.2"}}, true)).
				NotTo(HaveOccurred())
			Expect(graph.Add(baz, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.1.0"}}, true)).NotTo(HaveOccurred())
		...
		It("should return error for no matching version", func() {
			Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.0.0, < 1.1.1"}}, true)).
				NotTo(HaveOccurred())
			Expect(graph.Add(baz, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.1.0"}}, true)).NotTo(HaveOccurred())
		...
		It("should return correct version", func() {
			Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.0.0, < 1.1.1"}}, true)).
				NotTo(HaveOccurred())
			Expect(graph.Add(baz, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: ">= 1.1.0"}}, true)).
				NotTo(HaveOccurred())
	...
	Describe("Dependencies", func() {
		It("should return all dependencies", func() {
			Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: "1.x.x"}, {Name: baz}}, true)).
				NotTo(HaveOccurred())
	...
	Describe("Dependants", func() {
		It("should return all dependants", func() {
			Expect(graph.Add(foo, "v1.0.0", []v1alpha1.Dependency{{Name: bar, Version: "1.x.x"}}, true)).NotTo(HaveOccurred())
			Expect(graph.Add(baz, "v1.0.0", []v1alpha1.Dependency{{Name: bar}}, true)).NotTo(HaveOccurred())
			Expect(graph.Add(bar, "v1.0.0", nil, false)).NotTo(HaveOccurred())
			Expect(graph.Dependants(bar)).To(ConsistOf(foo, baz))
		})
	})
	...
	Describe("Constraints", func() {
		It("should return constraints of dependants", func() {
			...
			Expect(graph.Add(baz, "v1.0.0", []v1alpha1.Dependency{{Name: bar}}, true)).NotTo(HaveOccurred())
			Expect(graph.Add(bar, "v1.0.0", nil, false)).NotTo(HaveOccurred())
			Expect(graph.Constraints(bar)).To(ConsistOf(constraint("1.x.x")))
		})
	})
	...
	Describe("DeepCopy", func() {
		It("should produce equal graph", func() {
			...
			Expect(graph.Add(bar, "v1.1.0", nil, false)).NotTo(HaveOccurred())
