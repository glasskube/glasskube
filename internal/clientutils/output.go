...
func Format(outputFormat OutputFormat, showAll bool, pkgs ...ctrlpkg.Package) (string, error) {
	...
	case OutputFormatJSON:
		...
		for _, pkg := range pkgs {
			if pkgAsMap, pruneErr := pruneStatus(pkg, !showAll); pruneErr != nil {
	...
	case OutputFormatYAML:
		...
		l := len(pkgs)
		for _, pkg := range pkgs {
			if p, pruneErr := pruneStatus(pkg, !showAll); pruneErr != nil {
				return "", fmt.Errorf("failed to prune status during marshalling: %w", pruneErr)
...
func pruneStatus(pkg ctrlpkg.Package, doPrune bool) (map[string]any, error) {
